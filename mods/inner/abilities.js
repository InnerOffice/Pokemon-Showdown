...
let BattleAbilities = {
	...
	"aerilate": {
		...
		onBasePower: function (basePower, pokemon, target, move) {
			if (move.aerilateBoosted) return this.chainModify(1.5);
		},
		id: "aerilate",
		name: "Aerilate",
		rating: 4,
	...
	"flamebody": {
		shortDesc: "40% chance a Pokemon making contact with this Pokemon will be burned.",
		onAfterDamage: function (damage, target, source, move) {
			if (move && move.flags['contact']) {
	...
	"galvanize": {
		...
		onBasePowerPriority: 8,
		onBasePower: function (basePower, pokemon, target, move) {
			if (move.galvanizeBoosted) return this.chainModify(1.5);
		},
		id: "galvanize",
		name: "Galvanize",
		rating: 4,
	...
	"pixilate": {
		...
		onBasePowerPriority: 8,
		onBasePower: function (basePower, pokemon, target, move) {
			if (move.pixilateBoosted) return this.chainModify(1.5);
		},
		id: "pixilate",
		name: "Pixilate",
		rating: 4,
	...
	"refrigerate": {
		...
		onBasePowerPriority: 8,
		onBasePower: function (basePower, pokemon, target, move) {
			if (move.refrigerateBoosted) return this.chainModify(1.5);
		},
		id: "refrigerate",
		name: "Refrigerate",
		rating: 4,
	...
	"telepathy": {
		...
		onFoeTryMove: function (target, source, effect) {
			if ((source.side === this.effectData.target.side || effect.id === 'perishsong') && effect.priority > 0.1 && effect.target !== 'foeSide') {
				this.attrLastMove('[still]');
				this.add('cant', this.effectData.target, 'ability: Telepathy', effect, '[of] ' + target);
				return false;
			}
		},
		id: "telepathy",
